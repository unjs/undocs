...
Document options, props or parameters. A `::field` shows its `name` in monospace with the `type` as a badge, and the content beneath (or the `description` prop) as its body. Add `required` to flag it. Wrap fields in a `::field-group` to list them with dividers. Use `:br` for a line break inside a field.
...
::tabs
  ...
  ::field-group
    ...
    Storage prefix for cache keys. :br
    Defaults to `/cache`.
    ::
    ::field{name="maxAge" type="number"}
    Maximum age of a cache entry, in seconds.
    ::
    ::field{name="swr" type="boolean" description="Serve stale content while revalidating."}
    ::
  ::
  ::
  ::div
